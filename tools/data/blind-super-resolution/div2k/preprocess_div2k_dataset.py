...
def main(args):
    """A multi-thread tool to handle HR images to sub-images and their corresponding LQ images, and store
    the blur kernels.

    It is used for DIV2K dataset, especially used for DAN training.

    Warnings:
        It only support dataset with 'folder' mode, because we directly generate kernels in .mat files, and haven't
        write loading functions to read kernel from other mode like lmdb.
    ...
    opt (dict): Configuration dict. It contains:
        ...
        isotropic_rate (float): 1.0 for isotropic gaussian blur kernel(regular), 0.0 for anisotropic.
    ...
    # kernel related
    opt['isotropic_rate'] = args.isotropic_rate
...
def handle_subimages(opt):
    ...
    # write some description about kernel
    
    f = open(osp.join(opt['save_folder'], 'kernel_info.txt'), 'w')
    f.write(f'kernel size:{opt["kernel_size"]} \n'
            f'isotropic rate:{opt["isotropic_rate"]}, 0 for anisotropic kernel\n'
            f'min sigma:{opt["kernel_sigma_min"]} \n'
            f'max sigma:{opt["kernel_sigma_max"]} \n'
...
def worker(path, opt):
    ...
    for x in h_space:
        for y in w_space:
            ...
            kernel = generate_kernels(number=1,
                                      kernel_size=opt['kernel_size'],
                                      sigma_min=opt['kernel_sigma_min'],
                                      sigma_max=opt['kernel_sigma_max'],
                                      isotropic_rate=opt['isotropic_rate'],
...
def parse_args():
    ...
    parser.add_argument('--data-root', default='/data/xieyufeng/DIV2K/', help='dataset root')
    parser.add_argument(
        '--crop-size',
        nargs='?',
        default=256,
