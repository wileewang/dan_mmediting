...
class Degradation:
    ...
    def __init__(self, opt, key='gt'):
        self.opt = opt
        self.key = key

    @staticmethod
    def _generate_blur_kernel(opt):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        if isinstance(results[self.key], list):
            for v in results[self.key]:
                blur_kernel = self._generate_blur_kernel(self.opt)
                lq = cv2.filter2D(v, -1, blur_kernel)
                if 'gt_kernel' in results.keys() and 'lq' in results.keys():
                    results['kernel'].append(blur_kernel)
                    results['lq'].append(lq)
                else:
                    results['kernel']=[blur_kernel]
                    results['lq']=[lq]
        else:
            img = results[self.key]
            blur_kernel = self._generate_blur_kernel(self.opt)
            lq = cv2.filter2D(img, -1, blur_kernel)
            results['kernel'] = blur_kernel
            results['lq'] = lq
