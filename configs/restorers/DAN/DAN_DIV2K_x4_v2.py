...
train_cfg = None
test_cfg = dict(metrics=['PSNR'], crop_border=4, convert_to='y')

# dataset settings
train_dataset_type = 'BSRFolderDataset'
...
train_pipeline = [
    ...
    dict(type='Collect', keys=['lq', 'gt', 'kernel'], meta_keys=['lq_path', 'gt_path', 'kernel_path'])
]
...
data = dict(
    workers_per_gpu=1,
    train_dataloader=dict(samples_per_gpu=6, drop_last=True, pin_memory=True),  # 8 gpus
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            lq_folder='/data/xieyufeng/DIV2K/DIV2K_train_LQ_sub/x4',
            gt_folder='/data/xieyufeng/DIV2K/DIV2K_train_HR_sub',
            kernel_folder='/data/xieyufeng/DIV2K/DIV2K_train_kernel',
    ...
    val=dict(
        ...
        kernel_folder='/data/xieyufeng/DIV2K/DIV2KRK/gt_k_x4',
...
lr_config = dict(policy='Step', by_epoch=False, step=[50000, 100000, 200000, 300000], gamma=0.5)
...
# load_from = '/data/xieyufeng/dan_mmediting/work_dirs/DAN_DIV2k_x4/latest.pth'
resume_from = '/data/xieyufeng/dan_mmediting/work_dirs/DAN_DIV2k_x4/iter_180000.pth'
workflow = [('train', 1)]
